...
const revertAndUpdateWallets = async (
  ...
  try {
    const oldWalletSnapshot = await getDoc(
      doc(firebaseDb, "wallets", oldTransaction.walletId),
    );

    const originalWallet = oldWalletSnapshot.data() as WalletType;

    let newWalletSnapshot = await getDoc(
      doc(firebaseDb, "wallets", newWalletId),
    );
...
export const deleteTransaction = async (
  ...
  try {
    ...
    if (!transactionSnapshot.exists()) {
      return { success: true, msg: "Transaction not found!" };
    }
    ...
    const walletSnapshot = await getDoc(doc(firebaseDb, "wallets", walletId));
    const walletData = walletSnapshot.data() as WalletType;
...
export const fetchMonthlyStats = async (uid: string): Promise<ResponseType> => {
  try {
    ...
    // maping each transaction in month
    querySnapshot.forEach((doc) => {
      const transaction = doc.data() as TransactionType;
      transaction.id = doc.id;
      transactions.push(transaction);
      ...
      const transactionMonth = `${tDate.getFullYear()}-${String(tDate.getMonth() + 1).padStart(2, "0")}`;

      const monthData = monthlyData.find(
        (month) => month.fullDate == transactionMonth,
      );
